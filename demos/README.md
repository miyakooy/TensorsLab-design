# Demos

Visual examples, showcases, and case studies for TensorsLab Design.

## Entry points

- `index.html` — links to all demo pages
- `hero.html` — homepage hero direction
- `style.html` — visual style reference
- `design-canvas.html` — variant comparison board
- `frames.html` — device/window frame gallery
- `motion.html` — motion primitives sample
- `deck-stage.html` — keyboard-driven slide shell

These demos are intentionally small and concrete. They exist so the skill has real working references instead of only prose.
