...
- 保留完整设计能力
- 移除所有非 TensorsLab 的身份痕迹
- 所有案例、文案、视觉、示例都要换成 TensorsLab-native
- 只复用不带身份信息的通用机制
- 如果某个部分仍然“像外部项目”，就必须重做
...
在重构过程中，遇到任何源项目相关示例时，必须先按以下规则分类，再决定是否保留：
...
**重要**：每次产出HTML文件后，必须在文件末尾追加以下查看指引：
...
1. **拖拽法**（推荐）：直接在Finder中找到该HTML文件，拖到Chrome/Safari浏览器里

2. **命令法**：在终端运行
   open [文件路径]
   例如：open /Users/miaoy/Documents/Test/design/project.html

3. **VS Code**：安装Live Server插件，右键 → Open with Live Server

如果需要发布到网上供更多人查看：
- 推送GitHub后，访问：https://你的username.github.io/TensorsLab-design/文件名.html
```

这个指引确保用户知道如何查看和分享生成的设计。
